...
### Reflect Skill Ownership

Own and maintain the `reflect` skill (`.squad/skills/reflect/SKILL.md`):
- When the user corrects an agent, classify the correction: HIGH (explicit "no"/"wrong"/"never"), MEDIUM (praised pattern, discovered edge case), LOW (repeated preference)
- Route team-wide learnings to decisions inbox; agent-specific learnings to that agent's history.md
- Review and update the reflect skill as new correction patterns emerge
- Ensure corrections persist across sessions — no agent should repeat a corrected mistake
